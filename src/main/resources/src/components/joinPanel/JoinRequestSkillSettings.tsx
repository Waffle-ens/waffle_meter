    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-semibold text-current opacity-45">{label}</span>
          <button
            onClick={() => toggleGroup(codes, !allChecked)}
            className="cursor-pointer text-xs text-current opacity-40 transition-opacity hover:opacity-70">
          ...
          {codes.map((code) => (
            <Button
              ...
              className={cn(
                ...
                getClassColor(job, isLightOverlay ? "light" : "dark"),
      ...
      <div
        ...
        <div
          ...
          {GROUPED_BY_JOB.map(({ job, normalSkills, stigmaSkills }) => {
            ...
            return (
              <div key={job}>
                <div className="mb-3 flex items-center gap-2 pb-1 text-sm font-bold text-[var(--meter-fg)]">
                  <img
